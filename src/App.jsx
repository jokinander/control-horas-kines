...
export default function App() {
  ...
  useEffect(() => {
    const unsubData = onSnapshot(doc(db, "app", "data"), (snap) => {
      if (snap.exists()) setData(snap.data().value);
      setLoading(false);
    }, () => setLoading(false));

    const unsubRepl = onSnapshot(doc(db, "app", "replacements"), (snap) => {
      if (snap.exists()) setReplacements(snap.data().value);
    });

    return () => { unsubData(); unsubRepl(); };
  }, []);

  const save = useCallback(async (newData, newRepl) => {
    setSaving(true);
    try {
      await setDoc(doc(db, "app", "data"), { value: newData || data });
      await setDoc(doc(db, "app", "replacements"), { value: newRepl || replacements });
  ...
  const exportResumen = () => {
    ...
    text += `──────────────────────────────────\n  TOTAL GENERAL\n──────────────────────────────────\n`;
    text += `  Turnos disp. mes:   ${g.disponiblesMes}\n  Turnos disp. kine:  ${g.disponiblesKine}\n`;
    text += `  Turnos dados:       ${g.dados}\n  Turnos asistidos:   ${g.asistidos}\n`;
    text += `  Cancelados:         ${g.cancelados}\n  % Asistencia:       ${pct(g.asistidos, g.dados)}\n\n`;
    ...
    navigator.clipboard.writeText(text).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2500); }).catch(() => { window.prompt("Copiá este texto:", text); });
  };
  ...
  if (loading) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", fontFamily: "'DM Sans', sans-serif", background: "linear-gradient(135deg, #0f172a 0%, #1e293b 100%)" }}>
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: "linear-gradient(135deg, #0f172a 0%, #1e293b 100%)", minHeight: "100vh", color: "#e2e8f0", padding: "0" }}>
      ...
      <div style={{ background: "rgba(15,23,42,0.8)", borderBottom: "1px solid rgba(148,163,184,0.1)", padding: "16px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12, backdropFilter: "blur(12px)", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <img src="/logo.png" alt="Horas Agile VIP" style={{ width: 40, height: 40, borderRadius: 8, objectFit: "cover" }} />
          <div>
            <h1 style={{ margin: 0, fontSize: 16, fontWeight: 700, letterSpacing: "-0.02em" }}>Horas Agile VIP</h1>
      ...
      <div style={{ padding: "8px 24px 24px" }}>
        {view === "semanal" && (
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
              <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}><span style={{ color: kineObj.color }}>{kineObj.name}</span> — {MONTHS[month]}</h2>
              <span style={{ fontSize: 11, color: "#64748b", fontFamily: "'Space Mono', monospace", background: "rgba(100,116,139,0.1)", padding: "4px 8px", borderRadius: 6 }}>{kineObj.days.join(" · ")} | {kineObj.weekly}hs/sem</span>
            </div>
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, fontSize: 13 }}>
                <thead><tr><th style={{ ...thStyle, width: 180 }}>Métrica</th>{data[selectedKine][month].weeks.map((_, i) => (<th key={i} style={thStyle}>Sem {i + 1}</th>))}<th style={{ ...thStyle, color: "#facc15" }}>Total</th></tr></thead>
                <tbody>
                  {[{ key: "disponiblesMes", label: "Turnos disp. mes" }, { key: "disponiblesKine", label: "Turnos disp. kine" }, { key: "dados", label: "Turnos dados" }, { key: "asistidos", label: "Turnos asistidos" }].map(field => (
                    <tr key={field.key}>
                      ...
                      {data[selectedKine][month].weeks.map((w, wi) => (<td key={wi} style={cellStyle}><input type="number" min="0" value={w[field.key] || ""} placeholder="0" onChange={e => updateWeekField(selectedKine, month, wi, field.key, e.target.value)} style={inputStyle} /></td>))}
                      <td style={{ ...cellStyle, fontWeight: 700, fontFamily: "'Space Mono', monospace", color: "#facc15" }}>{data[selectedKine][month].weeks.reduce((s, w) => s + (w[field.key] || 0), 0)}</td>
                    </tr>
                  ))}
                  <tr><td style={{ ...labelStyle, color: "#f87171" }}>Cancelados</td>{data[selectedKine][month].weeks.map((w, wi) => (<td key={wi} style={{ ...cellStyle, fontFamily: "'Space Mono', monospace", color: "#f87171" }}>{w.dados - w.asistidos}</td>))}<td style={{ ...cellStyle, fontWeight: 700, fontFamily: "'Space Mono', monospace", color: "#f87171" }}>{data[selectedKine][month].weeks.reduce((s, w) => s + (w.dados - w.asistidos), 0)}</td></tr>
                  <tr><td style={{ ...labelStyle, color: "#60a5fa" }}>% Asistencia</td>{data[selectedKine][month].weeks.map((w, wi) => (<td key={wi} style={{ ...cellStyle, fontFamily: "'Space Mono', monospace", color: "#60a5fa" }}>{pct(w.asistidos, w.dados)}</td>))}<td style={{ ...cellStyle, fontWeight: 700, fontFamily: "'Space Mono', monospace", color: "#60a5fa" }}>{pct(data[selectedKine][month].weeks.reduce((s, w) => s + w.asistidos, 0), data[selectedKine][month].weeks.reduce((s, w) => s + w.dados, 0))}</td></tr>
                </tbody>
              </table>
            </div>
        ...
        {view === "mensual" && (
          <div>
            <h2 style={{ margin: "0 0 16px", fontSize: 18, fontWeight: 700 }}><span style={{ color: kineObj.color }}>{kineObj.name}</span> — Resumen {MONTHS[month]}</h2>
            {(() => { const t = getMonthTotals(selectedKine, month); return (
            ...
            {(() => { const t = getMonthTotals(selectedKine, month); const max = Math.max(t.disponiblesMes, 1); return (
              <div style={{ marginTop: 24 }}>
                {[{ label: "Disp. Mes", value: t.disponiblesMes, color: "#475569" }, { label: "Disp. Kine", value: t.disponiblesKine, color: "#2563eb" }, { label: "Dados", value: t.dados, color: "#7c3aed" }, { label: "Asistidos", value: t.asistidos, color: "#059669" }, { label: "Cancelados", value: t.cancelados, color: "#dc2626" }].map((b, i) => (
                  <div key={i} style={{ marginBottom: 8 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}><span style={{ color: "#94a3b8" }}>{b.label}</span><span style={{ fontFamily: "'Space Mono', monospace", color: b.color }}>{b.value}</span></div>
                    <div style={{ height: 8, borderRadius: 4, background: "rgba(51,65,85,0.4)" }}><div style={{ height: "100%", borderRadius: 4, background: b.color, width: `${(b.value / max) * 100}%`, transition: "width 0.5s ease" }} /></div>
                  </div>
                ))}
              </div>
            ); })()}
        ...
        {view === "general" && (
          <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
              ...
              <button onClick={exportResumen} style={{ padding: "8px 18px", borderRadius: 8, fontSize: 13, fontWeight: 700, background: copied ? "rgba(52,211,153,0.2)" : "linear-gradient(135deg, rgba(37,99,235,0.25), rgba(124,58,237,0.25))", border: copied ? "1px solid #34d39955" : "1px solid rgba(96,165,250,0.3)", color: copied ? "#34d399" : "#60a5fa", cursor: "pointer", fontFamily: "'DM Sans', sans-serif", transition: "all 0.3s" }}>{copied ? "✓ Copiado al portapapeles" : "📋 Exportar resumen"}</button>
            </div>
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, fontSize: 13 }}>
                <thead><tr><th style={thStyle}>Kine</th><th style={thStyle}>Disp. Mes</th><th style={thStyle}>Disp. Kine</th><th style={thStyle}>Dados</th><th style={thStyle}>Asistidos</th><th style={thStyle}>Cancelados</th><th style={thStyle}>% Asist.</th></tr></thead>
                <tbody>
                  {KINES.map(k => { const t = getMonthTotals(k.id, month); return (
                    <tr key={k.id}><td style={{ ...labelStyle, color: k.color }}>{k.name}</td><td style={monoCell}>{t.disponiblesMes}</td><td style={monoCell}>{t.disponiblesKine}</td><td style={monoCell}>{t.dados}</td><td style={{ ...monoCell, color: "#34d399" }}>{t.asistidos}</td><td style={{ ...monoCell, color: "#f87171" }}>{t.cancelados}</td><td style={{ ...monoCell, color: "#facc15" }}>{pct(t.asistidos, t.dados)}</td></tr>
                  ); })}
                  {(() => { const g = getGeneralMonth(month); return (
                    <tr style={{ borderTop: "2px solid #334155" }}><td style={{ ...labelStyle, fontWeight: 700, color: "#e2e8f0" }}>TOTAL</td><td style={{ ...monoCell, fontWeight: 700 }}>{g.disponiblesMes}</td><td style={{ ...monoCell, fontWeight: 700 }}>{g.disponiblesKine}</td><td style={{ ...monoCell, fontWeight: 700 }}>{g.dados}</td><td style={{ ...monoCell, fontWeight: 700, color: "#34d399" }}>{g.asistidos}</td><td style={{ ...monoCell, fontWeight: 700, color: "#f87171" }}>{g.cancelados}</td><td style={{ ...monoCell, fontWeight: 700, color: "#facc15" }}>{pct(g.asistidos, g.dados)}</td></tr>
                  ); })()}
                </tbody>
              </table>
            </div>
            {(replacements[month] || []).length > 0 && (
              <div style={{ marginTop: 20, padding: 16, borderRadius: 12, background: "rgba(251,191,36,0.05)", border: "1px solid rgba(251,191,36,0.15)" }}>
                ...
                {replacements[month].map((r, i) => (
                  <div key={i} style={{ display: "flex", gap: 12, fontSize: 13, marginBottom: 4 }}><span style={{ fontWeight: 600 }}>{r.name}</span>{r.reemplaza && <span style={{ color: "#94a3b8" }}>→ {r.reemplaza}</span>}<span style={{ fontFamily: "'Space Mono', monospace", color: "#fbbf24" }}>{r.hours}hs</span></div>
                ))}
                <div style={{ marginTop: 8, paddingTop: 8, borderTop: "1px solid rgba(251,191,36,0.2)", fontSize: 13, fontWeight: 700 }}>Total reemplazos: <span style={{ fontFamily: "'Space Mono', monospace", color: "#fbbf24" }}>{replacements[month].reduce((s, r) => s + r.hours, 0)}hs</span></div>
              </div>
            )}
          </div>
        )}
          ...
          <div>
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, fontSize: 12 }}>
                <thead><tr><th style={thStyle}>Mes</th><th style={thStyle}>Disp. Mes</th><th style={thStyle}>Disp. Kine</th><th style={thStyle}>Dados</th><th style={thStyle}>Asistidos</th><th style={thStyle}>Cancel.</th><th style={thStyle}>% Asist.</th><th style={thStyle}>Reempl.</th></tr></thead>
                <tbody>
                  {MONTHS.map((m, mi) => { const g = getGeneralMonth(mi); const replH = (replacements[mi] || []).reduce((s, r) => s + r.hours, 0); const hasData = g.disponiblesMes > 0 || g.dados > 0; return (
                    <tr key={mi} style={{ opacity: hasData ? 1 : 0.3 }}><td style={{ ...labelStyle, cursor: "pointer" }} onClick={() => { setMonth(mi); setView("general"); }}>{m}</td><td style={monoCell}>{g.disponiblesMes}</td><td style={monoCell}>{g.disponiblesKine}</td><td style={monoCell}>{g.dados}</td><td style={{ ...monoCell, color: "#34d399" }}>{g.asistidos}</td><td style={{ ...monoCell, color: "#f87171" }}>{g.cancelados}</td><td style={{ ...monoCell, color: "#facc15" }}>{pct(g.asistidos, g.dados)}</td><td style={{ ...monoCell, color: "#fbbf24" }}>{replH > 0 ? `${replH}hs` : "—"}</td></tr>
                  ); })}
                  {(() => { const totals = { dm: 0, dk: 0, d: 0, a: 0, c: 0, r: 0 }; MONTHS.forEach((_, mi) => { const g = getGeneralMonth(mi); totals.dm += g.disponiblesMes; totals.dk += g.disponiblesKine; totals.d += g.dados; totals.a += g.asistidos; totals.c += g.cancelados; totals.r += (replacements[mi] || []).reduce((s, r) => s + r.hours, 0); }); return (
                    <tr style={{ borderTop: "2px solid #334155" }}><td style={{ ...labelStyle, fontWeight: 700 }}>TOTAL ANUAL</td><td style={{ ...monoCell, fontWeight: 700 }}>{totals.dm}</td><td style={{ ...monoCell, fontWeight: 700 }}>{totals.dk}</td><td style={{ ...monoCell, fontWeight: 700 }}>{totals.d}</td><td style={{ ...monoCell, fontWeight: 700, color: "#34d399" }}>{totals.a}</td><td style={{ ...monoCell, fontWeight: 700, color: "#f87171" }}>{totals.c}</td><td style={{ ...monoCell, fontWeight: 700, color: "#facc15" }}>{pct(totals.a, totals.d)}</td><td style={{ ...monoCell, fontWeight: 700, color: "#fbbf24" }}>{totals.r > 0 ? `${totals.r}hs` : "—"}</td></tr>
                  ); })()}
                </tbody>
              </table>
            </div>
            <div style={{ marginTop: 24 }}>
              ...
              <div style={{ display: "flex", alignItems: "flex-end", gap: 4, height: 120 }}>
                {MONTHS.map((m, mi) => { const g = getGeneralMonth(mi); const maxAll = Math.max(...MONTHS.map((_, i) => getGeneralMonth(i).asistidos), 1); const h = (g.asistidos / maxAll) * 100; return (
